...
%
% This code generates a set of white or pink multi-sine signals for use in system
...
%   seed        seed for repeatable random phase (default: 0--"shuffle")
...
arguments
    ...
    options.seed (1,1) {mustBeFinite, mustBeReal, mustBeNonnegative} = 0
end
...
%% Discard some samples if signal is white; 
% help to reduce orthogonality testing time below

NewSampleSizeGain = 2;
if size(Amp_f,2) >= NewSampleSizeGain*(options.NumInput*options.NumExp) && options.color == "white"
    signal = signal(:,1:NewSampleSizeGain*(options.NumInput*options.NumExp));
    Amp_f = Amp_f(:,1:NewSampleSizeGain*(options.NumInput*options.NumExp));
...
%% 
% New time vector to repeat signal
